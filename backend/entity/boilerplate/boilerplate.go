...
type Boilerplate struct {
	...
	// Code uniqueness among non-deleted rows is enforced at the DB level by a
	// partial unique index (see migrations/0001_create_boilerplate.up.sql) and
	// checked at the service level — not via a plain GORM uniqueIndex, since
	// that would also block reusing the code of a soft-deleted row.
